...
class InquirerControl(FormattedTextControl):
    ...
    def _init_choices(self, choices):
        ...
        for i, c in enumerate(choices):
            if isinstance(c, Separator):
                ...
                self.choices.append((name, value, disabled, description, columns))
                self.counts[i] = count
        ...
        def append(index, line):
            if isinstance(line, Separator):
                ...
                if choice[2]:  # disabled
                    ...
                    if self.counts.get(index, None) is not None:
                        tokens.append(('', "{0:<5}".format(self.counts[index])))
                    else:
                        tokens.append(('', ''))
                    ...
                    if choice[3]:  # description
                        ...
                        tokens.append(('', '{0:<20}'.format(line_name), select_item))

                    if len(choice[4]) != 0:
                        for column in choice[4]:
                            tokens.append(('', '{0:<20}'.format(column)))
    ...
    def get_selected_values(self):
        ...
        return [((c[1],self.counts[i],c[4])  if len(c[4]) != 0 else (c[1],self.counts[i])) for i,c in enumerate(self.choices)
            if not isinstance(c, Separator) and self.counts[i] > 0]
...
def question(message, **kwargs):
    ...
    # TODO extract common parts for list, checkbox, rawlist, expand, orderlist
    # TODO validate
    if not 'choices' in kwargs:
        raise PromptParameterException('choices')
    # this does not implement default, use checked...
    if 'default' in kwargs:
        raise ValueError('Orderlist does not implement \'default\' '
    ...
    def get_prompt_tokens():
        ...
        if ic.answered:
            ...
            tokens.append(('class:instruction',
                           ' (<up>, <down> to move, <left> to decrement, <right> to increment)'))
    ...
    def move_cursor_right(event):
        def _increment():
            ic.counts[ic.pointer_index] += 1
        _increment()

    @kb.add('left', eager=True)
    def move_cursor_left(event):
        def _decrement():
            if ic.counts[ic.pointer_index] > 0:
                ic.counts[ic.pointer_index] -= 1
        _decrement()
